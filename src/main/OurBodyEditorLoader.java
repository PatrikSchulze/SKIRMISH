 ...
 */
public class OurBodyEditorLoader {
	...
	public OurBodyEditorLoader(FileHandle file) {
		if (file == null) throw new NullPointerException("file is null");
		model = readJson(file.readString());
	}

	public OurBodyEditorLoader(String str) {
	...
	{
		return (model.rigidBodies.get(in) != null);
	}
	 ...
	 */
	public void attachFixture(Body body, String name, FixtureDef fd, float scale, boolean flipVerti, boolean flipHori) {
		...
		for (int i=0, n=rbModel.polygons.size(); i<n; i++) {
			...
			for (int ii=0, nn=vertices.length; ii<nn; ii++) {
				...
				//yDown camera flip
				vertices[ii].y = 1f-vertices[ii].y;
				
				if (flipHori)  vertices[ii].x = 1f-vertices[ii].x;
				if (flipVerti) vertices[ii].y = 1f-vertices[ii].y;
			}
